...
Hippoclaudus includes a native macOS menu bar app (Swift/AppKit) and uses LaunchAgent for scheduling. The file layout must follow platform conventions so that macOS features (Time Machine exclusions, Spotlight indexing, permission dialogs) work correctly. Cross-platform portability is not a current goal.
...
- **Reverse DNS naming** (`com.chagui.hippoclaudus`): Follows Apple conventions, avoids collisions with other applications.
